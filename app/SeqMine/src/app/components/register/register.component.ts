...
export class RegisterComponent {
      ...
      return this.database.getUserByID(inputID).subscribe(
        result => {
          if (Object.keys(result).length > 0) {
            ...
            this.userRegistrationForm.controls['username'].setValue(foundUser.username);
            this.userRegistrationForm.controls['role'].setValue(foundUser.role_name);
            this.userRegistrationForm.controls['password'].setValue(foundUser.password);
            this.userRegistrationForm.controls['email'].setValue(foundUser.email);
            this.userRegistrationForm.controls['firstName'].setValue(foundUser.first_name);
            this.userRegistrationForm.controls['lastName'].setValue(foundUser.last_name);
          }
        }
      );
    }
    ...
    doRegisterUser() {
        return this.database.registerUser({
            ...
            next: result => { this.registrationResult = result.result; },
            error: error => { this.registrationResult = false; this.errorMessage += error.error.errorMsg}
        });
    }
